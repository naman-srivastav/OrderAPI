...
class OrdersController extends Controller
{
    ...
    public function __construct(OrderRepository $orderRepository, Order $orderModel)
    {
        ...
        try {
            ...
            if (!empty(array_get($validation_response, 'error', ''))) {
                return response()->json(
                    ...
                    array_get($validation_response, 'code', Response::HTTP_INTERNAL_SERVER_ERROR)
                );
            }
    ...
    public function create(Request $request)
    {
        try {
            ...
            $data = $this->orderModel->getDistance($request->get('origin'), $request->get('destination'));
            if (!empty(array_get($data, 'error', ''))) {
                return response()->json(['error' => $data['error']], Response::HTTP_NOT_FOUND);
            }
            $order = $this->orderRepository->create($data);
            return response()->json(
                ["id" => $order->id, "distance" => $order->distance, "status" => $order->status],
                Response::HTTP_OK
            );
        } catch (Exception $ex) {
            return response()->json(['error' => $ex->getMessage()], Response::HTTP_INTERNAL_SERVER_ERROR);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            } elseif ($order->status == Order::TAKEN) {
