...
class Order extends Model
{
    ...
    public function getDistance($origin, $destination)
    {
        ...
        $url = config('config.DISTANCE_MATRIX_URL')."&origins=".
                $lat1.",".$long1."&destinations=".$lat2.",".$long2."&key=".config('config.GOOGLE_MAP_KEY');
        $ch = curl_init();
        ...
        if (array_get($response, 'status', '') != "OK" ||
                empty(array_get($response, 'rows.0.elements.0.distance.value', ''))) {
